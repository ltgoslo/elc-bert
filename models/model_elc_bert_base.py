        ...
        for layer in self.layers:
            if self.activation_checkpointing:
                hidden_states.append(
                    checkpoint.checkpoint(layer, hidden_states, attention_mask, relative_embedding)
                )
            else:
                hidden_states.append(
...
class EncoderLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        #self.prev_layer_weights = nn.Parameter(torch.ones(layer_num+1))

    def forward(self, hidden_states, padding_mask, relative_embedding):
        #prev_layer_weights = F.leaky_relu(self.prev_layer_weights, negative_slope=1e-5)
        #prev_layer_weights = F.relu(self.prev_layer_weights)
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, relative_embedding):
        ...
        attention_scores = torch.bmm(query, key.transpose(1, 2) * self.scale)  # shape: [B, H, Tq, Tk]
