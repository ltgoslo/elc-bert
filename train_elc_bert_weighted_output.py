...
from pre_training.utils import cosine_schedule_with_warmup, is_main_process, get_rank, seed_everything, get_world_size
...
def parse_arguments():
    ...
    parser.add_argument("--output_dir", default="checkpoints/elc_bert_weighted_output", type=str, help="The output directory where the model checkpoints will be written.")
    ...
    parser.add_argument("--wandb_name", type=str, default="ELC BERT Weighted out")
    parser.add_argument("--wandb_project", type=str, default="ELC BERT")
...
def prepare_model_and_optimizer(args, device, local_rank, checkpoint):
    ...
    high_no = ["res"]
    decay_params = [(n, p) for n, p in model.named_parameters() if (not any(nd in n for nd in no_decay) and not any(hn in n for hn in high_no))]
    no_decay_params = [(n, p) for n, p in model.named_parameters() if any(nd in n for nd in no_decay)]
    high_no_decay_params = [(n, p) for n, p in model.named_parameters() if any(hn in n for hn in high_no)]
    optimizer_grouped_parameters = [
        {'params': [p for _, p in decay_params], 'weight_decay': args.weight_decay},
        {'params': [p for _, p in no_decay_params], 'weight_decay': 0.0},
        {'params': [p for _, p in high_no_decay_params], 'weight_decay': 0.0, "lr": args.learning_rate*1}
    ]
...
def training_epoch(model, tokenizer, data, optimizer, scheduler, grad_scaler, global_step, epoch, args, device, max_local_steps):
    ...
    for local_step, batch in enumerate(train_iter):
        ...
        with torch.cuda.amp.autocast(args.mixed_precision):
            ...
            loss = F.cross_entropy(prediction, target_ids, label_smoothing=args.label_smoothing) / args.gradient_accumulation
        ...
        if (local_step + 1) % args.gradient_accumulation == 0:
            ...
            if is_main_process():
                train_iter.set_postfix_str(f"loss: {total_loss:.2f}, accuracy: {avg_accuracy * 100.0:.2f}, grad_norm: {grad_norm:.2f}, lr: {optimizer.param_groups[0]['lr']:.5f}")
        ...
        if global_step == int(args.device_max_steps * args.long_after) and (local_step+1) % args.gradient_accumulation == 0:
            optimizer.zero_grad(set_to_none=True)
            return global_step
...
if __name__ == "__main__":
    ...
    for epoch in count(initial_epoch):
        ...
        global_step = training_epoch(model, tokenizer, train_data, optimizer, scheduler, grad_scaler, global_step, epoch, args, device, min_length)
